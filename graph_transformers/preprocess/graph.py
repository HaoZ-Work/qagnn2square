...
def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part2(data, model, tokenizer):
    # print(data)
...
def generate_adj_matrices(ori_schema_graph_path,
                          cpnet_graph_path,
                          cpnet_vocab_path,
                          output_path,
                          num_processes,
...
def generate_adj_data_from_grounded_concepts(
        grounded,
        cpnet_graph_path,
        cpnet_vocab_path,
        num_processes):
    ...
    This function will save
        ...
        (4) amask that specifies whether a node is a answer concept
    ...
    num_processes: int
    """
    print(f'generating adj data for {grounded}...')
...
def generate_adj_data_from_grounded_concepts__use_LM(
    ...
    for ex in grounded:
        ...
        QAcontext = "{} {}.".format(statement_obj['question'], ex['ans'])
    ...
    for j, _data in enumerate(res1):
        # if j % 100 == 0:
            # print(j)
        res2.append(concepts_to_adj_matrices_2hop_all_pair__use_LM__Part2(_data, model, tokenizer))
...
def coo_to_normalized_per_inst(data):
    ...
    i, j, k = np.concatenate((i, i + n_rel // 2), 0), np.concatenate((j, k), 0), np.concatenate((k, j), 0)
    adj_list = []
    for r in range(n_rel):
        mask = i == r
        ones = np.ones(mask.sum(), dtype=np.float32)
        # A is transposed by exchanging the order of j and k
        A = sparse.csr_matrix((ones, (k[mask], j[mask])), shape=(max_node_num, max_node_num))
...
def coo_to_normalized(adj_path, output_path, max_node_num, num_processes):
    ...
    adj_data = []
    with Pool(num_processes) as p:
        for i, (ori_adj_len, adj_len, concepts, adj_list, qm, am) in \
                tqdm(enumerate(p.imap(coo_to_normalized_per_inst, data)), total=len(data)):
...
def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part2_api(data, model, tokenizer):
    qc_ids, ac_ids, question, extra_nodes = data
    cid2score = get_LM_score_api(qc_ids+ac_ids+extra_nodes, question, model, tokenizer)
    return (qc_ids, ac_ids, question, extra_nodes, cid2score)
...
def generate_adj_data_from_grounded_concepts__use_LM_api(
        statement_json,
        grounded,
        cpnet_graph_path,
        cpnet_vocab_path,
    ...
    for ex in grounded:
        ...
        q_ids_api = set(concept2id_api(c) for c in ex['qc'])
        a_ids_api = set(concept2id_api(c) for c in ex['ac'])
    ...
    for j, _data in enumerate(res1_api):
        if j % 100 == 0: print (j)
        # res2.append(concepts_to_adj_matrices_2hop_all_pair__use_LM__Part2(_data))
        res2_api.append(concepts_to_adj_matrices_2hop_all_pair__use_LM__Part2_api(_data,model, tokenizer))
