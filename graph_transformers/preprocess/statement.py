...
def convert_to_entailment(input: dict):
    question_text = input["question"]
    choices = input["choices"]
    if "statements" not in input:
        input["statements"] = []
    for idx, choice in enumerate(choices):
        statement = create_hypothesis(question_text, choice)
        # Create the output dictionary from the input, premise and hypothesis statement
        input["statements"].append({"label": True if idx == 0 else False, "statement": statement})
    return input
